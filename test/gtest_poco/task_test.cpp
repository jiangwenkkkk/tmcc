...
//			sleep(10000);
		...
		int getnum()
		{
			...
			std::vector<int> all_visitpro =  {25,22,21,23,311,351,471,24,451,431,971,28,851,871,29,931,951,991,891,898,10,571,551,591,791,531,371,27,731,20,771};
...
TEST(TaskManagerTest,testCancel)
{
	TaskManager tm;
	TaskObserver to;
	tm.addObserver(Observer<TaskObserver, TaskStartedNotification>(to, &TaskObserver::taskStarted));
...
TEST(TaskManagerTest,testError)
{
	TaskManager tm;
	TaskObserver to;
	tm.addObserver(Observer<TaskObserver, TaskStartedNotification>(to, &TaskObserver::taskStarted));
...
TEST(TaskManagerTest,testCustomer)
{
	TaskManager tm;
...
TEST(TaskManagerTest,testMultiTasks)
{
	TaskManager tm;
...
TEST(TaskManagerTest,testCustomThreadPool)
{
	ThreadPool  tp(2, 6, 120);
	TaskManager tm(tp);
...
TEST(TaskManagerTest,testCustomThreadPool_getnum)
{
	ThreadPool  tp(2, 6, 120);
